document.addEventListener("DOMContentLoaded", () => {
    ...
    // ==========================================
    // 1. CAPTURA DE IP, DISPOSITIVO, LOCALIZAÇÃO E DISCORD (COM FALLBACK SEGURO)
    // ==========================================
    async function registarAcesso() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                provedor = data.org || provedor;
            } else {
                throw new Error("Falha na primeira API, tentando fallback...");
            }
        } catch (e) {
            // TENTATIVA 2 (FALLBACK): ip-api.com (Caso a primeira falhe ou seja bloqueada por Adblocks)
            try {
                const fallbackResponse = await fetch('https://demo.ip-api.com/json/?lang=pt-BR'); // Usando endpoint público seguro
                const data = await fallbackResponse.json();
                userIP = data.query || userIP;
                pais = data.country || pais;
                estado = data.regionName || estado;
                cidade = data.city || cidade;
                provedor = data.isp || provedor;
            } catch (fallbackError) {
                // TENTATIVA 3 (FALLBACK ADICIONAL): ipify + ip-api direto
                try {
                    const ipResponse = await fetch('https://api.ipify.org?format=json');
                    const ipData = await ipResponse.json();
                    userIP = ipData.ip;
                    
                    const geoResponse = await fetch(`http://ip-api.com/json/${userIP}?lang=pt-BR`);
                    const geoData = await geoResponse.json();
                    pais = geoData.country || pais;
                    estado = geoData.regionName || estado;
                    cidade = geoData.city || cidade;
                    provedor = geoData.isp || provedor;
                } catch (lastError) {
                    console.warn("Todas as APIs de geolocalização foram bloqueadas pelo navegador/adblock.");
                }
            }
        }
        ...
        try {
            ...
            console.warn("Não foi possível enviar a notificação para o Discord.");
        }
    }
